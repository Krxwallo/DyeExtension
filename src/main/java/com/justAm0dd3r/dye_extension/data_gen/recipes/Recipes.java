...
public class Recipes extends RecipeProvider {
            ...
            switch (property.getType()) {
                ...
                case DataGenerationProperty.Type.BUTTON:
                    ShapelessRecipeBuilder
                            ...
                            .addIngredient(ReflectionUtils.requireBlockHolderByName(StringUtils.getDyedBaseName(property.getDyedName())).getBlock().get())
    ...
    private Block getByProperty(DataGenerationProperty property) {
        switch (property.getType()) {
            case DataGenerationProperty.Type.STAIRS:
                return ReflectionUtils.requireDyedBlockHolderByName(property.getDyedName()).getBlockFromColor(property.getDyeColor()).getStairsBlock().get();
            case DataGenerationProperty.Type.SLAB:
                return ReflectionUtils.requireDyedBlockHolderByName(property.getDyedName()).getBlockFromColor(property.getDyeColor()).getSlabBlock().get();
            case DataGenerationProperty.Type.BUTTON:
                return ReflectionUtils.requireDyedBlockHolderByName(property.getDyedName()).getBlockFromColor(property.getDyeColor()).getButtonBlock().get();
            default:
                return ReflectionUtils.requireDyedBlockHolderByName(property.getDyedName()).getBlockFromColor(property.getDyeColor()).getBlock().get();
        }
    }
    ...
    private IItemProvider getByResult(String result, int ingredient) {
        switch (ingredient) {
            case RecipeIngredients.DYE:
                return ReflectionUtils.requireItemByName(StringUtils.getDye(result));
            case RecipeIngredients.DYED_BLOCK:
                return ReflectionUtils.requireDyedBlockByName(result);
            default:
                // Block
                return ReflectionUtils.getBlockByName(StringUtils.getBaseName(result));
        }
    }
}
